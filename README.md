...
- 新增「舊資料帳務校正」：針對早期版本造成的成本/已實現口徑差額建立可逆校正層，不改原始交易。
- 權息管理升級為「企業行動」，新增現金增資。
- 現金增資可記錄認購價、可認股數、實際認購股數、繳款日、撥股日。
- 繳款日反映現金支出；撥股日後自動增加庫存與成本，並形成可指定賣出的獨立「現金增資」批次。
- 雲端/本機備份包含舊資料校正紀錄，Storage schema 升級為 v7。


## v5.1 除息與增資介面微調
- 「企業行動」使用者介面名稱改為「除息與增資」。
- 現金增資新增與除權息相同的股票代號/名稱搜尋與自動帶入。
- 舊資料校正改為置中、保留上下留白的固定高度對話框；內容過多時在彈窗內滾動。


## v5.2 首頁精簡儀表板
- 首頁保留總資產、總損益、總報酬率、現金、持股市值、已/未實現損益等核心資訊。
- 次要分析改為可收合區塊：投資分析、資產與獲利用途、帳務與資料檢查。
- 資料健康與帳務差額只有在異常時於首頁主動提示。
- 保留原有族群、獲利池、舊資料校正與診斷功能。

## v5.3 - K棒技術判讀
- 新增獨立「K棒技術判讀」工具，桌面工具列與手機「更多」皆可進入。
- 個股完整損益中心可直接帶入股票代號啟動分析。
- 使用 Yahoo Finance 約一年日 K OHLCV 歷史行情。
- 判讀項目：最新 K 棒型態、5/10/20/60 日均線位置、量價關係、短中期趨勢、近端支撐與壓力、最近 10 根 K 棒型態。
- 綜合結果只做技術訊號彙整，畫面明確提示不代表未來必然走勢。


## v5.4 - 互動式 K 線技術圖
- K棒技術判讀新增最近約 100 個交易日的互動式日 K 線圖。
- 圖上疊加 MA5、MA10、月線 MA20、季線 MA60 與成交量/20日均量。
- 可點選或滑過任一根 K 棒查看日期、開高低收、漲跌幅、量能、均線位置與辨識型態。
- 支撐/壓力直接以虛線標示於價格圖，重要 K 棒型態以訊號點呈現。
- 移除原本最近 10 根 K 棒表格，讓判讀集中在圖形與下方文字解讀。
