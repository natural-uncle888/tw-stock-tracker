...
createApp({
    ...
    <div v-if="isLoggedIn" class="app-shell max-w-7xl mx-auto p-4 md:p-8 pb-32" @click="closeContextMenu" @contextmenu.prevent>
        ...
        <button type="button"
            class="save-status-indicator cloud-status-pill floating-position-target"
            :class="[cloudBackupStatusClass, floatingControlClass('cloud')]"
            :style="floatingControlStyle('cloud')"
            :title="floatingEditMode ? '拖曳調整雲端備份狀態位置' : cloudBackupStatusTitle"
            @pointerdown="startFloatingControlDrag($event, 'cloud')"
            @click.stop="floatingEditMode ? null : handleCloudStatusClick()">
            <span v-if="floatingEditMode" class="floating-position-handle"><i class="fa-solid fa-up-down-left-right"></i></span>
            <span class="cloud-status-pill__main">
                <i :class="cloudBackupStatusIcon"></i>
                <span>{{ cloudBackupStatusText }}</span>
                <span v-if="cloudBackupNeedsAttention" class="cloud-status-pill__dot"></span>
            </span>
            <span class="cloud-status-pill__sub">{{ cloudBackupStatusSubtext }}</span>
        </button>
...
<div class="card relative z-10 !pb-10 border-t-4" :class="newTx.type === 'buy' ? 'border-t-blue-500' : 'border-t-rose-500'">
            ...
            <div v-if="!showStockDetails && holdings.length" class="mb-5 space-y-3">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 px-2">
                    <div>
                        ...
                        <div class="text-xs md:text-sm font-bold text-slate-500 mt-1">整合報酬率、法人籌碼、融資融券、當沖熱度與注意／處置風險。</div>
                    </div>
                ...
                <div v-if="portfolioRiskRadar" class="rounded-3xl border p-4 md:p-5 shadow-sm" :class="portfolioRiskRadar.className">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <div class="text-xs font-bold bg-white/70 border border-current/10 rounded-2xl px-3 py-2">提示：預設會把較需處理的持股放前面；拖曳卡片或按上下箭頭可改成自訂順序。</div>
                        <button v-if="hasCustomHoldingOrder" type="button" @click="resetHoldingCardOrder" class="text-xs font-black bg-white/80 border border-current/10 rounded-2xl px-3 py-2 hover:bg-white transition"><i class="fa-solid fa-arrow-rotate-left mr-1"></i>恢復預設排序</button>
                    </div>
                </div>
                ...
                <div class="p-5 md:p-7">
                    ...
                    <div v-if="categoryEditStock" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div><label class="text-xs font-black text-slate-500 mb-2 block">大類股</label><div class="flex gap-2"><input v-model="categoryEditForm.sector" list="sector-options" class="input-field flex-1" placeholder="例如：電子"><button type="button" @click="quickAddCategoryMasterItem('sectors', categoryEditForm.sector)" class="w-12 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 hover:bg-emerald-50 hover:text-emerald-600" title="加入大類股清單"><i class="fa-solid fa-plus"></i></button></div><datalist id="sector-options"><option v-for="item in categoryMaster.sectors" :key="'sector-'+item" :value="item"></option></datalist></div>
                            <div><label class="text-xs font-black text-slate-500 mb-2 block">細產業</label><div class="flex gap-2"><input v-model="categoryEditForm.industry" list="industry-options" class="input-field flex-1" placeholder="例如：PCB"><button type="button" @click="quickAddCategoryMasterItem('industries', categoryEditForm.industry)" class="w-12 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 hover:bg-emerald-50 hover:text-emerald-600" title="加入細產業清單"><i class="fa-solid fa-plus"></i></button></div><datalist id="industry-options"><option v-for="item in categoryMaster.industries" :key="'industry-'+item" :value="item"></option></datalist></div>
                        </div>
                        <div><label class="text-xs font-black text-slate-500 mb-2 block">主族群</label><div class="flex gap-2"><input v-model="categoryEditForm.mainTheme" list="theme-options" class="input-field flex-1" placeholder="例如：PCB / AI伺服器 / 散熱"><button type="button" @click="quickAddCategoryMasterItem('themes', categoryEditForm.mainTheme)" class="w-12 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 hover:bg-emerald-50 hover:text-emerald-600" title="加入題材族群清單"><i class="fa-solid fa-plus"></i></button></div><datalist id="theme-options"><option v-for="item in categoryMaster.themes" :key="'theme-main-'+item" :value="item"></option></datalist><div class="text-[11px] font-bold text-slate-400 mt-1">Dashboard 會用主族群計算曝險與損益。</div></div>
            ...
            floatingPositions: (() => {
                try {
                    ...
                    return {
                        ...
                        cloud: parsed.cloud && Number.isFinite(Number(parsed.cloud.x)) && Number.isFinite(Number(parsed.cloud.y)) ? { x: Number(parsed.cloud.x), y: Number(parsed.cloud.y) } : null
                    };
                } catch (_) { return { price: null, cloud: null }; }
            })(),
    ...
    methods: {
        ...
        toggleFloatingEditMode() {
            if (this.floatingEditMode) {
                ...
                this.openInfoModal('位置已鎖定', '更新股價與雲端備份狀態膠囊的位置已儲存，之後不會再被拖動。');
                return;
            }
            this.floatingEditMode = true;
            this.$nextTick(() => this.ensureFloatingControlPositions());
        ...
        ensureFloatingControlPositions() {
            ['price', 'cloud'].forEach(type => {
                if (this.floatingPositions && this.floatingPositions[type]) return;
                const selector = type === 'price' ? '.floating-price-update' : '.save-status-indicator.cloud-status-pill';
        ...
        resetFloatingControlPositions() {
            this.floatingPositions = { price: null, cloud: null };
