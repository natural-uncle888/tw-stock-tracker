...
createApp({
    ...
    <div v-if="isLoggedIn" class="app-shell max-w-7xl mx-auto p-4 md:p-8 pb-32" @click="closeContextMenu" @contextmenu.prevent>
        ...
        <div class="save-status-indicator" :class="saveStatusClass" :title="saveStatusTitle">
            ...
            <div class="w-full flex flex-col gap-2">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <button @click="openPortfolioModal"
                            class="mobile-portfolio-btn max-w-[52vw] h-10 px-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-700 font-black text-sm flex items-center gap-2 active:scale-95 transition shadow-sm">
                ...
                <div class="flex gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    <button @click="showSecurityModal = true" class="w-10 h-10 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center active:bg-slate-200 transition-colors" title="安全性"><i class="fa-solid fa-lock"></i></button>
                    <button @click="showSettings = true" class="w-10 h-10 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center active:bg-slate-200" title="設定"><i class="fa-solid fa-gear"></i></button>
                    ...
                    <button @click="openCommodityModal" class="w-10 h-10 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center active:bg-slate-200 transition-colors" title="商品期貨"><i class="fa-solid fa-coins"></i></button>
        ...
        <header class="desktop-header hidden md:flex flex-col xl:flex-row justify-between mb-8 gap-6 bg-white/80 backdrop-blur-xl rounded-[2rem] p-6 shadow-[0_20px_60px_-24px_rgba(15,23,42,0.28)] border border-white/90">
            <div class="brand-block flex items-center gap-5">
            ...
            <div class="tab-switch" role="tablist" aria-label="主功能分頁">
                <button @click="currentTab = 'dashboard'; showStockDetails = false" type="button" role="tab" :aria-selected="currentTab === 'dashboard'" :class="['tab-btn', currentTab === 'dashboard' ? 'active' : '']">
                ...
                <button @click="currentTab = 'cash'; showStockDetails = false" type="button" role="tab" :aria-selected="currentTab === 'cash'" :class="['tab-btn', currentTab === 'cash' ? 'active' : '']">
                    <i class="fa-solid fa-money-bill-transfer text-base"></i><span>資金流</span>
                </button>
            </div>

            <div class="header-actions">
                <button @click="openPortfolioModal" class="btn btn-secondary portfolio-header-btn !h-12 !px-4 !rounded-xl" title="投資組合／自訂帳本"><i class="fa-solid fa-users text-lg"></i><span class="hidden md:inline portfolio-header-name font-bold text-slate-700">{{ currentPortfolio.name }}</span></button>
                ...
                <button @click="openCommodityModal" class="btn btn-secondary !h-12 !px-4 !rounded-xl" title="商品期貨"><i class="fa-solid fa-coins text-lg"></i></button>
            ...
            <div class="bg-white w-full max-w-4xl rounded-[2rem] shadow-2xl relative flex flex-col max-h-[90vh]">
                ...
                <div class="overflow-y-auto p-6 space-y-5">
                    <div class="bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm text-slate-600 leading-relaxed">
                        ...
                        這裡提供黃金、白銀、銅、布蘭特原油的即時報價小工具（TradingView）。因交易所授權限制，嵌入版通常無法直接顯示部分交易所期貨連續合約，故改用 TradingView 指標／差價合約報價作為即時參考。若畫面顯示空白，請確認瀏覽器未封鎖第三方小工具後再試。
                    </div>
                    ...
                    <div class="rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4 shadow-sm">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
                            ...
                            <button @click="fetchTaiexFuturesInstitutionalOi" :disabled="institutionalOiLoading" class="h-11 px-4 rounded-xl font-extrabold text-white bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                                ...
                                {{ institutionalOiLoading ? '查詢中…' : '查詢最新資料' }}
                            </button>
                        </div>
...
<p><strong>7) 備份：</strong>右上角 <i class="fa-solid fa-download mx-1"></i> 下載備份檔，或用 <i class="fa-solid fa-cloud mx-1"></i> Google 雲端備份（需先填 OAuth Client ID）。備份包含：交易、費率、帳本、分類、股名對照、資金流等。</p>
        </div>
    </div>
    ...
    <div>A：通常是你先記了買入但尚未入金，或曾經出金/提領導致不足。到「資金流」新增入金，或用系統提示的「一鍵補入金」把現金補到非負即可。</div>
        ...
        <div class="md:hidden fixed bottom-0 left-0 w-full z-40 bg-white/95 backdrop-blur border-t border-slate-200 px-4 pt-2 pb-[calc(env(safe-area-inset-bottom)+10px)]">
            <div class="grid grid-cols-4 gap-2">
                <button type="button" @click="currentTab = 'dashboard'; showStockDetails = false" class="flex flex-col items-center justify-center py-2 rounded-2xl font-black transition-all"
                    :class="currentTab === 'dashboard' ? 'bg-slate-900 text-white border border-slate-700 shadow-sm' : 'text-slate-600 hover:bg-slate-50'">
                ...
                <button type="button" @click="currentTab = 'cash'; showStockDetails = false" class="flex flex-col items-center justify-center py-2 rounded-2xl font-black transition-all"
                    :class="currentTab === 'cash' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 shadow-sm' : 'text-slate-600 hover:bg-slate-50'">
...
const savedCash = localStorage.getItem(window.StockStorage.KEYS.cashBook) || ''; if (savedCash) { try { const parsed = JSON.parse(savedCash); this.cashBook = Array.isArray(parsed) ? parsed : []; } catch(_) { this.cashBook = []; } }
    ...
    beforeUnmount() { window.StockSaveStatus.clearTimer(this); window.removeEventListener('click', this.handleGlobalClick, true); window.removeEventListener('resize', this.closeContextMenu); window.removeEventListener('scroll', this.closeContextMenu, true); },
    computed: {
