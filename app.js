...
<div class="card relative z-10 !pb-10 border-t-4" :class="newTx.type === 'buy' ? 'border-t-blue-500' : 'border-t-rose-500'">
            ...
            <div v-if="holdings.length" class="mt-5 mb-5 space-y-3">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 px-2">
                    <div>
                        <h2 class="text-xl font-black text-slate-800 flex items-center gap-2"><i class="fa-solid fa-radar text-cyan-600"></i> 持股分析雷達摘要</h2>
                        <div class="text-xs md:text-sm font-bold text-slate-500 mt-1">持股列表優先顯示；這裡只放摘要，需要時再展開完整分析。</div>
                    </div>
                    <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                        ...
                        <button @click="setPortfolioRadarExpanded(!portfolioRadarExpanded)" class="h-11 px-4 rounded-2xl font-black bg-white border-2 border-slate-200 text-slate-700 hover:bg-slate-50 active:scale-[0.98] flex items-center justify-center gap-2 shadow-sm">
                            <i class="fa-solid" :class="portfolioRadarExpanded ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
                            {{ portfolioRadarExpanded ? '收合詳細分析' : '展開詳細分析' }}
                        </button>
                    </div>
                </div>
                ...
                <div v-if="portfolioRiskRadar" class="rounded-3xl border p-4 md:p-5 shadow-sm" :class="portfolioRiskRadar.className">
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 lg:min-w-[420px]">
                            <div class="rounded-2xl bg-white/70 border border-current/10 px-3 py-2"><div class="text-[11px] font-bold opacity-70">偏正向</div><div class="font-black text-lg text-up">{{ portfolioRiskRadar.positive }}</div></div>
                            <div class="rounded-2xl bg-white/70 border border-current/10 px-3 py-2"><div class="text-[11px] font-bold opacity-70">偏弱</div><div class="font-black text-lg text-down">{{ portfolioRiskRadar.weak }}</div></div>
                            <div class="rounded-2xl bg-white/70 border border-current/10 px-3 py-2"><div class="text-[11px] font-bold opacity-70">高風險</div><div class="font-black text-lg text-amber-600">{{ portfolioRiskRadar.highRisk }}</div></div>
                            <div class="rounded-2xl bg-white/70 border border-current/10 px-3 py-2"><div class="text-[11px] font-bold opacity-70">待查</div><div class="font-black text-lg text-slate-600">{{ portfolioRiskRadar.missing }}</div></div>
                        </div>
                    </div>
                    <div class="mt-3 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                        <div class="text-xs font-bold bg-white/70 border border-current/10 rounded-2xl px-3 py-2">提示：完整分析會把較需處理的持股放前面；拖曳卡片或按上下箭頭可改成自訂順序。</div>
                ...
                <div v-show="portfolioRadarExpanded" class="grid grid-cols-1 lg:grid-cols-2 gap-3">
...
<div>
    ...
    data() {
        ...
        return {
            ...
            portfolioRadarExpanded: localStorage.getItem('tw_stock_portfolio_radar_expanded_v1') === 'true',
...
const savedCash = localStorage.getItem(window.StockStorage.KEYS.cashBook) || ''; if (savedCash) { try { const parsed = JSON.parse(savedCash); this.cashBook = Array.isArray(parsed) ? parsed : []; } catch(_) { this.cashBook = []; } }
    ...
    methods: {
        setPortfolioRadarExpanded(expanded) {
            this.portfolioRadarExpanded = !!expanded;
            try { localStorage.setItem('tw_stock_portfolio_radar_expanded_v1', this.portfolioRadarExpanded ? 'true' : 'false'); } catch (_) {}
        },
