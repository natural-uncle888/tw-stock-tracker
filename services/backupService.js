(function(window) {
  ...
  window.StockBackupService = {
            ...
            async refreshGDriveCloudMeta() { this.gdriveBusy = true; this.gdriveBusyText = '正在取得雲端狀態…'; try { const accessToken = await this._ensureGDriveAccessToken(); const info = await this._findBackupFileId(accessToken); if (!info) { this._writeCloudMeta({ cloudFileModifiedTime: '', cloudFileExists: false, lastCloudErrorAt: '', lastCloudErrorMessage: '' }); this.openInfoModal('雲端狀態', '雲端目前沒有找到備份檔（tw_stock_backup.json）。'); return; } this._writeCloudMeta({ cloudFileExists: true, cloudFileModifiedTime: info.modifiedTime || '', lastCloudErrorAt: '', lastCloudErrorMessage: '' }); this.openInfoModal('雲端狀態', '雲端最後修改時間。'); } catch(e) { this._writeCloudMeta({ lastCloudErrorAt: new Date().toISOString(), lastCloudErrorMessage: e?.message || String(e || '雲端狀態刷新失敗') }); this.openInfoModal('雲端狀態', `發生未預期錯誤：${e?.message || e}`); } finally { this.gdriveBusy = false; this.gdriveBusyText = ''; } },
    
            getDefaultGDriveClientId() { return __getDefaultGDriveClientId(); },

            getGDriveEffectiveClientId() { const custom = (this.gdriveClientId || '').trim(); return custom || __getDefaultGDriveClientId(); },

            getGDriveClientIdSource() { return (this.gdriveClientId || '').trim() ? 'custom' : (__getDefaultGDriveClientId() ? 'default' : 'none'); },

            saveGDriveClientId() { const cid = (this.gdriveClientIdInput || '').trim(); if (!cid) { this.openInfoModal('提示', '不填也可以使用網站內建的預設 Client ID；只有想改用自己的 Google Cloud 專案時才需要輸入。'); return; } localStorage.setItem(window.StockStorage.KEYS.gdriveClientId, cid); this.gdriveClientId = cid; this.gdriveClientIdInput = cid; __gdriveTokenClient = null; __gdriveTokenClientCid = null; this.openInfoModal('已儲存', '自訂 Google OAuth Client ID 已儲存，之後會優先使用這組設定。'); },
    
            clearGDriveClientId() { localStorage.removeItem(window.StockStorage.KEYS.gdriveClientId); this.gdriveClientId = ''; this.gdriveClientIdInput = ''; __gdriveTokenClient = null; __gdriveTokenClientCid = null; const hasDefault = !!__getDefaultGDriveClientId(); this.openInfoModal('已清除', hasDefault ? '已清除自訂 Client ID，之後會改用網站內建的預設 Client ID。' : '已清除 Google OAuth Client ID。目前尚未設定網站預設 Client ID。'); },
    
            async _ensureGDriveAccessToken() { const cid = this.getGDriveEffectiveClientId(); if (!cid) throw new Error('缺少 Google OAuth Client ID。請在 index.html 內設定網站預設 Client ID，或在安全性設定中輸入自訂 Client ID。'); if (!window.google || !google.accounts || !google.accounts.oauth2) { throw new Error('Google Identity Services 尚未載入，請稍後再試。'); } if (!__gdriveTokenClient || __gdriveTokenClientCid !== cid) { __gdriveTokenClient = google.accounts.oauth2.initTokenClient({ client_id: cid, scope: __GDRIVE_SCOPE, callback: () => {} }); __gdriveTokenClientCid = cid; } const token = await new Promise((resolve, reject) => { let finished = false; const timer = setTimeout(() => { if (finished) return; finished = true; reject(new Error('授權逾時。請確認已允許彈出視窗（Popup），並完成帳戶選擇／驗證後再試一次。')); }, 120000); __gdriveTokenClient.callback = (resp) => { if (finished) return; finished = true; clearTimeout(timer); if (resp && resp.access_token) resolve(resp.access_token); else reject(new Error(resp?.error_description || resp?.error || '授權失敗')); }; try { // ✅ 不強制每次都 consent，避免反覆觸發 Google 的再次驗證流程
